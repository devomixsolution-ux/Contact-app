...
const AdminPanel: React.FC<AdminPanelProps> = ({ lang }) => {
  ...
  useEffect(() => {
    if (view === 'details' && selectedMadrasah?.id) {
      fetchDataForMadrasah(selectedMadrasah.id);
    }
  }, [view, selectedMadrasah?.id]);

  const fetchDataForMadrasah = async (id: string) => {
  ...
  const fetchStudentCount = async (madrasahId: string) => {
    ...
    try {
      // Use explicit count logic for RLS compatibility
    ...
    <div className="animate-in slide-in-from-right-4 duration-500 pb-20 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-black text-white font-noto">প্রোফাইল ডিটেইলস</h1>
        </div>
        {!isEditing && (
          <button onClick={() => fetchDataForMadrasah(selectedMadrasah.id)} className="p-2 bg-white/10 text-white rounded-lg">
            <RefreshCw size={14} className={loadingCount || loadingDevices ? 'animate-spin' : ''} />
          </button>
        )}
      </div>
        ...
        {isEditing ? (
          <div className="space-y-4 text-left">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={saveChanges} className="flex-1 py-3 bg-green-500 text-white rounded-xl font-bold">সেভ</button>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/10 p-5 rounded-[2rem] border border-white/20 shadow-inner">
                ...
                <div className="text-2xl font-black text-white">
                  {loadingCount ? <Loader2 className="animate-spin mx-auto text-white/50" size={20} /> : studentCount}
                </div>
              ...
              <div className="bg-white/10 p-5 rounded-[2rem] border border-white/20 shadow-inner">
                ...
                <div className="text-2xl font-black text-white">
                  {loadingDevices ? <Loader2 className="animate-spin mx-auto text-white/50" size={20} /> : devices.length}
                </div>
            ...
            <div className="space-y-3 text-left">
              <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest ml-2">সক্রিয় ডিভাইসসমূহ</p>
                ...
                <div key={d.id} className="flex items-center justify-between bg-white/5 p-3.5 rounded-2xl border border-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-white/40">{new Date(d.last_active).toLocaleString('bn-BD')}</p>
                    </div>
                  </div>
                  <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"></div>
                </div>
              )) : (
                <p className="text-center text-white/20 text-xs py-4">কোনো ডিভাইস লগইন নেই</p>
              )}
            </div>
